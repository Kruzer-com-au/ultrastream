...
interface WarpTunnelProps {
  ...
  isMobile?: boolean; // mobile performance mode
}
...
function getAdaptiveParticleCount(mobile?: boolean): number {
  if (typeof window === 'undefined') return 1500;
  if (mobile) return 500; // Hard cap for mobile
...
/**
 * Creates the tunnel fragment shader with quality level.
 * Desktop: 4-octave FBM + domain warping + voronoi + chromatic distortion
 * Mobile:  2-octave FBM + simplified noise + no chromatic distortion
 */
function createTunnelFragmentShader(mobile: boolean): string {
  const FBM_OCTAVES = mobile ? 2 : 4;
  ...
  void main() {
    ...
    // Plasma base
    ${mobile
      ? 'float plasma = fbm(spiralUV * 2.0 + uTime * 0.2);'
    ...
    ${mobile ? '' : `
    // Chromatic distortion (desktop only - expensive)
...
// ============================================================
// HELPER: Compute speed from progress
// ============================================================
...
function TunnelWalls({
  ...
  // Mobile: fewer geometry segments (32 radial, 64 height vs 64/128)
  ...
  const fragmentShader = useMemo(
    () => createTunnelFragmentShader(mobile),
    [mobile]
  );
...
// ============================================================
// TUNNEL LIGHTING (reduced on mobile)
// ============================================================

function TunnelLighting({ progress, mobile }: { progress: number; mobile: boolean }) {
  ...
  return (
    <>
      <pointLight
        ...
        decay={2}
      />
      {/* Purple light skipped on mobile to save GPU */}
      {!mobile && (
        <pointLight
          ref={purpleRef}
          color="#7b2ff7"
...
function WarpTunnelScene({ progress, mobile }: { progress: number; mobile: boolean }) {
  const speed = getSpeed(progress);
  const particleCount = useMemo(() => getAdaptiveParticleCount(mobile), [mobile]);

  return (
    <>
      <CameraController progress={progress} />
      <TunnelLighting progress={progress} mobile={mobile} />

      <ambientLight intensity={0.03} />

      {/* Fog: lighter on mobile */}
      <fog attach="fog" args={["#020208", mobile ? 8 : 5, 50]} />

      <TunnelWalls progress={progress} speed={speed} mobile={mobile} />
      <SpeedStreaks progress={progress} speed={speed} particleCount={particleCount} />
...
export function WarpTunnel({ progress, visible, isMobile = false }: WarpTunnelProps) {
  const [dpr, setDpr] = useState<[number, number]>(
    isMobile ? [1, 1.2] : [1, 2]
  );

  const handleIncline = useCallback(() => {
    setDpr(isMobile ? [1, 1.2] : [1, 2]);
  }, [isMobile]);
      ...
      >
        <Suspense fallback={null}>
          <PerformanceMonitor
            ...
            bounds={(refreshrate) =>
              isMobile
                ? [refreshrate * 0.4, refreshrate * 0.7]
                : [refreshrate * 0.5, refreshrate * 0.8]
            }
          />
          <WarpTunnelScene progress={progress} mobile={isMobile} />
        </Suspense>
      </Canvas>
    </div>
  );
